        ...
        for sat in self.satellites : 
            ...
            if not load.exists(self.tle_dir+"/"+name) or load.days_old(self.tle_dir+"/"+name) >= self.max_days:
                if hasattr(sat, 'norad') : 
                    download_adress = config.tle.tle_download_adress + '?CATNR=' + urllib.parse.quote(str(sat.norad)) + '&FORMAT=TLE'
                else : 
                    download_adress = config.tle.tle_download_adress + '?NAME=' + urllib.parse.quote(str(sat.name)) + '&FORMAT=TLE'
                
                load.download(download_adress, filename = self.tle_dir+"/"+name)
...
if __name__ == "__main__":
    ...
    downloader = Tle_Downloader(config.satellites, config.directories.tle_dir,config.tle.max_days)
